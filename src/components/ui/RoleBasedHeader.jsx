...
const RoleBasedHeader = ({
  ...
  const getActualUserRole = () => {
    ...
    let detectedRole = 'member'; // default

    // Check currentUser.role first
    if (currentUser?.role) {
    ...
    // Check localStorage for stored user role
    try {
    ...
    // If no role found anywhere, check if this is the first user (likely owner)
    if (detectedRole === 'member') {
      // For now, assume logged-in users are owners until proper role system is implemented
          ...
          <div className='relative' ref={userDropdownRef}>
            ...
            >
              ...
              <div className='hidden md:block text-left'>
                ...
                <div className='text-xs text-text-secondary'>
                  {actualUserRole.charAt(0).toUpperCase() + actualUserRole.slice(1)}
                </div>
              </div>
              <Icon
                name='ChevronDown'
